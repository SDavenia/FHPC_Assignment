...
do
    ...
    do
        ...
        do
            ...
            do
                srun -n1 --cpus-per-task=12 ./gemm_"$implem"_"$type".x $m_size $m_size $m_size > output_close_mkl.txt #just a temporary file
                # Extract information using grep and regular expressions
                size=$m_size
                times=$(grep -o 'Time: [0-9.]*' output_close_mkl.txt| cut -d' ' -f2)
                gflops=$(grep -o 'GFLOPS: [0-9.]*' output_close_mkl.txt| cut -d' ' -f2)
...
rm output_close_mkl.txt # Delete the temporary file
