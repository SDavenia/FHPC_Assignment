...
out_filename=results/evolve_ordered/openMP_16_64.csv # To write times
...
do
    ...
    do
        ...
        for j in {1..5..1}
        do
            mpirun -n 4 --map-by $MAPBY --bind-to $BINDTO ./main_parallel.exe -r -k $ksize -e 0 -f $filename -n $nsteps -s $s  > output_ordered_openMP_16_64.txt
            time_value=$(grep -o 'Ordered time: [0-9.]*' output_ordered_openMP_16_64.txt | awk '{print $3}')
            echo "$ksize,$n_threads,$time_value" >> $out_filename
        done
    done
done

rm output_ordered_openMP_16_64.txt # Remove useless temporary file
