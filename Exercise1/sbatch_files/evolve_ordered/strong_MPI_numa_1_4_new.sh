...
out_filename=results/evolve_ordered/strong_MPI_numa_1_4_new.csv # To write times
...
do
    ...
    do
        for j in {1..5..1}
        do
            mpirun -n $n_processes --map-by $MAPBY ./main_parallel.exe -r -k $ksize -e 0 -f $filename -n $nsteps -s $s > output_ordered_strong_MPI_numa_1_4.txt
