...
out_filename=results/evolve_static/strong_MPI_numa_10000_new.csv # To write times
...
do
    ...
    do
        for j in {1..5..1}
        do
            mpirun -n $n_processes --map-by $MAPBY ./main_parallel.exe -r -k $ksize -e 1 -f $filename -n $nsteps -s $s > output_static_strong_MPI_numa_10000.txt
