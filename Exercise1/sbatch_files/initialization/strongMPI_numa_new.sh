...
out_generate=results/initialization/strong_MPI_numa_generate_new_10_16.csv # To write times
out_write=results/initialization/strong_MPI_numa_write_new_10_16.csv # To write times
...
do
    ...
    do
        for j in {1..5..1}
        do
            mpirun -n $n_processes --map-by $MAPBY ./main_parallel.exe -i -k $ksize -f $filename > output_initialization_strong_MPI_numa.txt
