...
out_filename=results/evolve_black_white/openMP_20000.csv # To write times
...
do
    ...
    do
        ...
        for j in {1..5..1}
        do
            mpirun -n 1 --map-by $MAPBY ./main_parallel.exe -r -k $ksize -e 2 -f $filename -n $nsteps -s $s  > output_black_white_openMP_20000.txt
            time_value=$(grep -o 'BlackWhite time: [0-9.]*' output_black_white_openMP_20000.txt | awk '{print $3}')
            echo "$ksize,$n_threads,$time_value" >> $out_filename
        done
    done
done

rm output_black_white_openMP_20000.txt # Remove useless temporary file
